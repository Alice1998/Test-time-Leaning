...
class LanguageModel:
    def __init__(self,
        ...
        if self.model_name in [
            ...
            "together_ai/deepseek-ai/DeepSeek-V3-0324",
    ...
    def getModelFeedback(self, message, temperature, max_tokens, use_model = ''):
        if use_model == "":
            use_model = self.model_name
        stream_flag = False
        if use_model == 'deepseek-v3-250324':
            stream_flag = True
        for attempt in range(20):
            try:
                ...
                print("- - -"*6)
                return output_message, reason_content
            except Exception as e:
                print(f"Error (attempt {attempt + 1}/20)")
                print(e)
    ...
    def generate(self,
        ...
        output,reason = self.getModelFeedback(message=history, temperature=temperature, max_tokens=max_tokens)
    ...
    def generate_multi_actor(self,
        ...
        if approach_name == "DynamicCheatsheet_Cumulative" or approach_name == 'DynamicCheatsheet_Cumulative_hasReward':
            ...
            for round in range(max(1, max_num_rounds)):
                ...
                if generator_answer == "No final answer found":
                    # generator_output = self.generate(
                    #     history=generator_history,
                    #     temperature=temperature,
                    #     max_tokens=max_tokens,
            ...
            return {
                "input_txt": input_txt,
                'input_instruction':generator_prompt,
                "steps": steps,
    ...
    def generate_multi_reflection(self,
        approach_name: str,
        game_content: str,
        env_feedback: str,
        cheatsheet: str = None,
        ...
        if approach_name == 'default':
            result_dict = {
                'input_game_content':game_content,
                'input_env_feedback': env_feedback,
                'input_cheatsheet': "",
                'input': "",
                'output': "",
                'cheatsheet': ""
        ...
        current_cheatsheet = cheatsheet
        cheatsheet_prompt = cheatsheet_template.replace("[[GAME_CONTENT]]", game_content).replace("[[FEEDBACK]]", env_feedback).replace("[[PREVIOUS_CHEATSHEET]]", current_cheatsheet)
        ...
        result_dict = {
            'input_game_content':game_content,
            'input_env_feedback': env_feedback,
            'input_cheatsheet': current_cheatsheet,
            'input':cheatsheet_prompt,
            'output':cheatsheet_output,
    ...
    def advanced_generate(self,
        ...
        if approach_name == "default":
        ...
        elif approach_name == "DynamicCheatsheet_Cumulative" or approach_name == 'DynamicCheatsheet_Cumulative_hasReward':
            if cheatsheet is None:
                raise ValueError("Cheatsheet must be provided for dynamic_cheatsheet approach.")
            if cheatsheet_template is None:
                raise ValueError("Cheatsheet template must be provided for dynamic_cheatsheet approach.")
            ...
            for round in range(max(1, max_num_rounds)):
                ...
                if "[[MODEL_ANSWER]]" in cheatsheet_template:
                    # single turn
                    ...
                    if approach_name == "DynamicCheatsheet_Cumulative_hasReward":
                        if generator_answer == target_answer:
                            reward = "Your answer is correct!"
                        else:
                            reward = f'Your answer is NOT correct! The answer is {target_answer}'
                        cheatsheet_prompt = cheatsheet_prompt.replace('[[ANSWER_REWARD]]', reward)
                # else:
                #     # multi turn
                #     cheatsheet_prompt = cheatsheet_template.replace("[[GAME_CONTENT]]", game_context).replace("[[FEEDBACK]]", game_feedback).replace("[[PREVIOUS_CHEATSHEET]]", current_cheatsheet)
