...
def eval_TwentyQuestion(output, target, candidate_word_list, model):
    counter = 0
    output_list = re.split(r'[^a-zA-Z]+', output.lower())
    hit_word_list = []
    for word in candidate_word_list:
        if ' ' in word:
            if word.lower() in output.lower():
                hit_word_list.append(word.lower())
                counter += 1
        else:
            if word.lower() in output_list:
                counter += 1
                hit_word_list.append(word.lower())
    # if counter >= 2:
    #     print("[Env]\nhit=",counter)
    ...
    result = 'No'
    state = False
    if target.lower() in hit_word_list:
        if counter == 1:
            print(f"[Env] feedback=1, you hit the right word {target}")
            return True, f"Yes\nYou are correct! The answer word is {target}."
        print("[HUMAN DOUBLD-CHECK] multiple hit!")
        print(output)
        print("target word:",target)
        print(hit_word_list)
        import ipdb; ipdb.set_trace()
        return state, result
    # if return_flag or state:
    #     print(f"[Env] feedback={state}, result = {result}")
    #     return state, result

    # if counter == 1:
    #     if target.lower() == hit_word_list[0]:
    #         print(f"[Env] feedback=1, you hit the right word {target}")
    #         return True, f"Yes\nYou are correct! The answer word is {target}."
    #     print("[HUMAN DOUBLD-CHECK] single untarget hit!")
    #     print(f"[Info]", hit_word_list)
    #     # print(output)
    #     # return False, "No"
    # if counter > 1:
    #     print("[HUMAN DOUBLD-CHECK] multiple hit!")
    ...
    prompt = f"Word: {target}. Question about the Word: {output}\nPlease answer the question with Yes/No/Invalid (if the question is not a yes/no question)."
    message = [{"role":"assistant",'content':"You are a helpful assistant."},{"role": "user", "content": prompt}]
    output, reason = model.getModelFeedback(message, temperature = 0, max_tokens = 100) # use_model = 'deepseek-v3-250324'
    # print("[Env]\n",output)
    output_word_list = re.split(r'[^a-zA-Z]+', output.lower())
    result_state = ['Yes',"No",'Invalid']
    result_list = []
    for word in result_state:
        if word.lower() in output_word_list:
            result_list.append(word)
    if len(result_list) == 1:
        print("[Env] feedback=",result_list[0])
        return False, result_list[0]
